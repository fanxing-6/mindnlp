...
def prepare_layoutlm_batch_inputs():
    ...
    input_ids = mindspore.tensor([[101, 1019, 1014, 1016, 1037, 12849, 4747, 1004, 14246, 2278, 5439, 4524, 5002, 2930,
                                  ...
                                  [101, 4070, 4034, 7020, 1024, 3058, 1015, 1013, 2861, 1013, 6070, 19274, 2772, 6205,
                                   27814, 16147, 16147, 4343, 2047, 10283, 10969, 14389, 1012, 2338, 102]],
                                 )  # noqa: E231
    ...
    labels = mindspore.tensor(
        [[-100, 10, 10, 10, 9, 1, -100, 7, 7, -100, 7, 7, 4, 2, 5, 2, 8, 8, -100, -100, 5, 0, 3, 2, -100],
         [-100, 12, 12, 12, -100, 12, 10, -100, -100, -100, -100, 10, 12, 9, -100, -100, -100, 10, 10, 10, 9, 12, -100,
          10, -100]], dtype=mindspore.int32)  # noqa: E231
    # fmt: on
...
class LayoutLMModelIntegrationTest(unittest.TestCase):
    ...
    def test_forward_pass_no_head(self):
        ...
        # self.assertTrue(numpy.allclose(outputs.last_hidden_state[0, :3, :3].asnumpy(),
        #                                expected_slice.asnumpy(), atol=1e-3))

        # test the pooled output on [1, :3]
        expected_slice = mindspore.tensor([-0.6580, -0.0214, 0.8552])

        # self.assertTrue(numpy.allclose(outputs.pooler_output[1, :3].asnumpy(),
        #                                expected_slice.asnumpy(), atol=1e-3))
    ...
    def test_forward_pass_sequence_classification(self):
        ...
        outputs = model(
            ...
            token_type_ids=token_type_ids,
            labels=mindspore.tensor([1, 1], dtype=mindspore.int32),
        )
